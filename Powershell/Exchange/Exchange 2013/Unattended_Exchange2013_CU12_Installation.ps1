<#
    .TITLE
    Brad's Ultimate Exchange Server 2013 CU12 Automation Script
    
    .SYNOPSIS
    Installs Exchange Server 2013 CU12 Client Access and Mailbox Roles and all required prerequisites.
    ...
    .DESCRIPTION
    This script will install Exchange 2013 CU12 prerequisites, creates the Exchange 
    ...
    1.2 Added check for domain membership of the server that is to receive the 
        ...
        Updated the script to include CU12 instead of CU10
...
#>
   ...
   #####################################################################################
    ...
    Write-Host `n `t "Exchange 2013 CU12 will install in the directory:" -ForegroundColor "Yellow" -NoNewLine 
    ...
    Function Create-ExtractFolder {
        $TargetFolder = Test-Path "C:\Exchange_Resources\Exchange_2013_CU11_Extracted"
        If($TargetFolder) {
            $TargetFolder = "C:\Exchange_Resources\Exchange_2013_CU11_Extracted"
    ...
    Function Reboots {
     ...
     # Sets Content for InstallExchange.ps1 script upon 2nd reboot to run on startup
            Set-Content $Path {
                ...
                $ExchangeInstallLocation = Get-Content -Path "C:\Exchange_Resources\InstallPath.txt" 
                Write-Host "Server is now ready to install Exchange Server 2013 CU12..."
   ...
   #####################################################################################
                ...
                C:\Exchange_Resources\Exchange_2013_CU11_Extracted\setup.exe /PrepareSchema /IAcceptExchangeServerLicenseTerms
                C:\Exchange_Resources\Exchange_2013_CU11_Extracted\setup.exe /PrepareAD /OrganizationName:"$Organization" /IAcceptExchangeServerLicenseTerms                     
                C:\Exchange_Resources\Exchange_2013_CU11_Extracted\setup.exe /mode:Install /role:ClientAccess,Mailbox /TargetDir:"$ExchangeInstallLocation" /OrganizationName:"$Organization" /IAcceptExchangeServerLicenseTerms       
   ...
   #####################################################################################
    ...
    Set-AutomaticLogon
    $Downloaded_Title = "" 
    $Downloaded_Message = "Have you already downloaded Exchange CU12?"
    ...
    $Downloaded = $host.ui.PromptForChoice($Downloaded_Title, $Downloaded_Message, $Downloaded_Options, 0) 
        switch ($Downloaded) {
            0 {
                Write-Host `n
                $Extracted_Title = "" 
                $Extracted_Message = "Has the Exchange CU12 executable already been extracted?"
                ...
                switch ($Extracted) {
                    0 {
                        :OuterLoop Do {
                            Write-Host `n "Please select the location where Exchange 2013 CU12 is extracted"
                            $ExchangeExtracted = Read-FolderBrowserDialog -Message "Exchange 2013 CU12 Extracted Directory"                                                      
                            If (![string]::IsNullOrEmpty($ExchangeExtracted)) { 
                                Write-Host "You selected the directory: $ExchangeExtracted" -Foreground "Yellow" `n
                                $ExchangeSetup = $ExchangeExtracted + "\Exchange2013-x64-cu12.exe"
                            }
                            Else { 
                                Clear-Host
                        ...
                        :OuterLoop Do {
                            Write-Host `n "Please select the location where you would like to have Exchange 2013 CU12 Installed" `n
                            $ExchangeInstallLocation = Read-FolderBrowserDialog -Message "Exchange 2013 CU12 Install Directory"
                    ...
                    1 {
                        :OuterLoop Do {
                            $ExchangeExtracted = Test-Path "C:\Exchange_Resources\Exchange_2013_CU11_Extracted"
                            If($ExchangeExtracted -eq $false) {
                                New-Item "C:\Exchange_Resources\Exchange_2013_CU11_Extracted" -type directory
                                $ExchangeExtracted = "C:\Exchange_Resources\Exchange_2013_CU11_Extracted"                                              
                            }
                        ...
                        :OuterLoop Do {
                            Clear-Host
                            Write-Host `n "Please choose the directory where the download for Exchange 2013 CU12 is located" `n
                            ...
                            If (![string]::IsNullOrEmpty($ExchangeDownload)) { 
                                ...
                                $ExchangeSetup = $ExchangeDownload + "\Exchange2013-x64-cu12.exe"
                            }
                            Else {
                            Clear-Host
                            Write-Host `n "You did not select a location to download Exchange 2013 CU12. Please select a location." -foreground "Red" `n
            ...
            1 {
                ...
                While ($ExchangeInstallLocation -eq $null)
                :OuterLoop Do { 
                    Write-Host `n "Please choose a directory to store the download for Exchange 2013 CU12" `n
                    $ExchangeDownload = Read-FolderBrowserDialog -Message "Please select a directory"
                    If (![string]::IsNullOrEmpty($ExchangeDownload)) { 
                        ...
                        Package-Download "https://download.microsoft.com/download/2/C/1/2C151059-9B2A-466B-8220-5AE8B829489B/Exchange2013-x64-cu12.exe" "$ExchangeSetup" 
                        Write-Host "Finished downloading Exchange2013-x64-cu12.exe"
                    }
                    Else { 
                        Clear-Host
