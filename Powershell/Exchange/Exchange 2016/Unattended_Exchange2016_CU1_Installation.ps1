<#
    .TITLE
    Brad's Ultimate Exchange Server 2016 CU1 Automation Script
    
    .SYNOPSIS
    Installs Exchange Server 2016 CU1 Client Access and Mailbox Roles and all required prerequisites.
    ...
    .DESCRIPTION
    This script will install Exchange 2016 CU1 prerequisites, creates the Exchange 
    ...
    1.2 Added check for domain membership of the server that is to receive the 
        ...
        Updated the script to include CU1 2016 instead of 2013
...
#>
   ...
   #####################################################################################
    ...
    Write-Host `n `t "Exchange 2016 CU1 will install in the directory:" -ForegroundColor "Yellow" -NoNewLine 
    ...
    Function Create-ExtractFolder {
        $TargetFolder = Test-Path "C:\Exchange_Resources\Exchange_2016_CU1_Extracted"
        If($TargetFolder) {
            $TargetFolder = "C:\Exchange_Resources\Exchange_2016_CU1_Extracted"
    ...
    Function Reboots {
     ...
     # Sets Content for InstallExchange.ps1 script upon 2nd reboot to run on startup
            Set-Content $Path {
                ...
                $ExchangeInstallLocation = Get-Content -Path "C:\Exchange_Resources\InstallPath.txt" 
                Write-Host "Server is now ready to install Exchange Server 2016 CU1..."
   ...
   #####################################################################################
                ...
                C:\Exchange_Resources\Exchange_2016_CU1_Extracted\setup.exe /PrepareSchema /IAcceptExchangeServerLicenseTerms
                C:\Exchange_Resources\Exchange_2016_CU1_Extracted\setup.exe /PrepareAD /OrganizationName:"$Organization" /IAcceptExchangeServerLicenseTerms                     
                C:\Exchange_Resources\Exchange_2016_CU1_Extracted\setup.exe /mode:Install /role:Mailbox /TargetDir:"$ExchangeInstallLocation" /OrganizationName:"$Organization" /IAcceptExchangeServerLicenseTerms       
   ...
   #####################################################################################
    ...
    Set-AutomaticLogon
    $Downloaded_Title = "" 
    $Downloaded_Message = "Have you already downloaded Exchange 2016 CU1?"
    ...
    $Downloaded = $host.ui.PromptForChoice($Downloaded_Title, $Downloaded_Message, $Downloaded_Options, 0) 
        switch ($Downloaded) {
            0 {
                Write-Host `n
                $Extracted_Title = "" 
                $Extracted_Message = "Has the Exchange 2016 CU1 executable already been extracted?"
                ...
                switch ($Extracted) {
                    0 {
                        :OuterLoop Do {
                            Write-Host `n "Please select the location where Exchange 2016 CU1 is extracted"
                            $ExchangeExtracted = Read-FolderBrowserDialog -Message "Exchange 2016 CU1 Extracted Directory"                                                      
                        ...
                        :OuterLoop Do {
                            Write-Host `n "Please select the location where you would like to have Exchange 2016 CU1 Installed" `n
                            $ExchangeInstallLocation = Read-FolderBrowserDialog -Message "Exchange 2016 CU1 Install Directory"
                    ...
                    1 {
                        :OuterLoop Do {
                            $ExchangeExtracted = Test-Path "C:\Exchange_Resources\Exchange_2016_CU1_Extracted"
                            If($ExchangeExtracted -eq $false) {
                                New-Item "C:\Exchange_Resources\Exchange_2016_CU1_Extracted" -type directory
                                $ExchangeExtracted = "C:\Exchange_Resources\Exchange_2016_CU1_Extracted"                                              
                            }
                        ...
                        :OuterLoop Do {
                            Clear-Host
                            Write-Host `n "Please choose the directory where the download for Exchange 2016 CU1 is located" `n
                            ...
                            Else {
                            Clear-Host
                            Write-Host `n "You did not select a location to download Exchange 2016 CU1. Please select a location." -foreground "Red" `n
            ...
            1 {
                ...
                While ($ExchangeInstallLocation -eq $null)
                :OuterLoop Do { 
                    Write-Host `n "Please choose a directory to store the download for Exchange 2016 CU1" `n
                    $ExchangeDownload = Read-FolderBrowserDialog -Message "Please select a directory"
                    If (![string]::IsNullOrEmpty($ExchangeDownload)) { 
